...
export default function UserModal({ user, mode, onClose, onSave, canEdit }) {
    ...
    const fetchData = async () => {
      try {
        ...
        const [sRes, eRes, enRes] = await Promise.all([
          ...
          axios.get(`https://albercas.onrender.com/api/users/${user.id}/enrollments`)
        ]);
  ...
  const handleRenew = async () => {
    ...
    try {
      ...
      const res = await axios.post('https://albercas.onrender.com/api/create-preference', {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await axios.get(`https://albercas.onrender.com/api/payments/check/${user.id}`);
