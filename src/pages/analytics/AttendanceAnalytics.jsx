...
import { mockAttendanceByHour, mockAttendanceByDay } from '../../data/mockData';
...
export default function AttendanceAnalytics() {
  ...
  const generate = async () => {
    setLoading(true);
    setInsights(null);
    const result = await aiAnalyticsService.generateInsights(mockAttendanceByHour, mockAttendanceByDay);
    setLoading(false);
    if (result.success) {
  ...
  const hourlyData = {
    labels: mockAttendanceByHour.labels,
    datasets: [{
      data: mockAttendanceByHour.data,
      backgroundColor: mockAttendanceByHour.data.map(v =>
        v > 55 ? 'rgba(99,102,241,0.85)' : 'rgba(99,102,241,0.4)'
      ),
      borderRadius: 6,
      borderSkipped: false,
    }],
  ...
  const weeklyData = {
    labels: mockAttendanceByDay.labels,
    datasets: [{
      data: mockAttendanceByDay.data,
      backgroundColor: mockAttendanceByDay.data.map(v =>
        v > 180 ? 'rgba(34,211,238,0.85)' : 'rgba(34,211,238,0.4)'
      ),
      borderRadius: 6,
      borderSkipped: false,
    }],
        ...
        <div className="card">
          <div className="card-header">
            <div>
              ...
              <div className="card-subtitle">Promedio visitantes/hora</div>
            </div>
      ...
      {!insights && !loading && (
        <div className="card" style={{ textAlign: 'center', padding: 'var(--space-12)' }}>
          ...
          <div style={{ fontSize: 14, color: 'var(--color-text-muted)', marginBottom: 'var(--space-6)', maxWidth: 420, margin: '0 auto var(--space-6)' }}>
            Presiona "Generar Análisis" para obtener insights automáticos sobre patrones de asistencia, tendencias de ingresos y recomendaciones operativas.
          </div>
